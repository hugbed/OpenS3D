...
TEST(histogram, min) {
  ...
  EXPECT_DOUBLE_EQ(Histogram<double>::Min(v), 0.0);
}
...
TEST(histogram, max) {
  ...
  v.resize(10);
  std::iota(std::begin(v), std::end(v), 0.0);
  EXPECT_DOUBLE_EQ(Histogram<double>::Max(v), 9.0);
}
...
TEST(histogram, bin_size_trivial) {
  ...
  EXPECT_DOUBLE_EQ(Histogram<double>::BinSize(min, max, nbBins), 1.0);
}
...
TEST(histogram, bin_size_less_trivial) {
  ...
  EXPECT_NEAR(Histogram<double>::BinSize(min, max, nbBins), 4.09, 0.01);
}
