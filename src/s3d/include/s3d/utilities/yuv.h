...
std::tuple<uint8_t, uint8_t, uint8_t> yuv_to_rgb(uint8_t Y, uint8_t U, uint8_t V)
{
    static_assert (
...
static std::vector<uint8_t> color_conversion(const std::vector<uint8_t>& in, YUV422, RGB8)
{
    uint8_t R, G, B;
...
static void color_conversion(InputIt first, InputIt last, OutputIt d_first, YUV422, RGB8)
{
    using std::is_same;
    using std::distance;
