...
#include "vertex.h"
#include "rendering/entity/billboard/billboardentity.h"
...
void OpenGLWidget::initEntities()
{
    ...
    if (m_textures.empty() || m_textures[0] == nullptr) return;
    ...
    billboardEntity->setPoints({QVector2D(0.0f, 0.0f),
                                QVector2D(w / 2.0f, h / 2.0f),
                                QVector2D(w - 1.0f, h - 1.0f)},
                               {-1.0f, 0.0f, 0.8f});
