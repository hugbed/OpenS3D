...
BillboardEntity::BillboardEntity(const QSize &imageSize)
...
{
    m_object.destroy();
    if (m_vertex.isCreated()) m_vertex.destroy();
    m_program.reset();
}
...
{
    m_object.release();
    if (m_vertex.isCreated()) m_vertex.release();
    m_program->release();
}
