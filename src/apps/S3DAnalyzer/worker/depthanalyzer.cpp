...
//  ::displayInNewWindow("yo", leftOrig);
  ...
  cv::resize(leftOrig, leftOrig, cv::Size2f(static_cast<float>(leftOrig.cols) / resizeRatio,
                                            static_cast<float>(leftOrig.rows) / resizeRatio), cv::INTER_AREA);
  cv::resize(rightOrig, rightOrig, cv::Size2f(static_cast<float>(rightOrig.cols) / resizeRatio,
                                              static_cast<float>(rightOrig.rows) / resizeRatio), cv::INTER_AREA);
...
//  std::cout << model;
  ...
  s3d::center_values(std::begin(bestPts1h), std::end(bestPts1h), std::begin(bestPts1h), -imageCenter);
  s3d::center_values(std::begin(bestPts2h), std::end(bestPts2h), std::begin(bestPts2h), -imageCenter);
