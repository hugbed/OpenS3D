...
PointCloudMesh::PointCloudMesh(const std::string& name, const std::string& resourcegroup,
                                 const std::vector<float> &points, const std::vector<float> &colors)
{
    ...
    vbuf = Ogre::HardwareBufferManager::getSingleton().createVertexBuffer(
            Ogre::VertexElement::getTypeSize(Ogre::VET_FLOAT3), msh->sharedVertexData->vertexCount, Ogre::HardwareBuffer::HBU_DYNAMIC_WRITE_ONLY_DISCARDABLE);
    ...
    if(colors.size() > 0)
    {
        ...
        for(int i=0, k=0; i<numpoints*3, k<numpoints; i+=3, k++)
        {
            ...
            rs->convertColourValue(Ogre::ColourValue(colors[i], colors[i+1], colors[i+2]), &colorsRGBA[k]);
        }
...
void PointCloudMesh::updateVertexPositions(int size, const std::vector<float> &points)
{
...
void PointCloudMesh::updateVertexColours(int size, const std::vector<float> &colours)
{
